  ...
  foreach ($data as $key => $row) {
    ...
    for ($j = 0; $j < 6; $j++) {
      ...
      if ($status == 0) {
        ...
        if ($j == 3) {
          $actext = "Compression Test Passed!";
        } else {
          $actext = "Accepted!";
        }
    ...
    $penalty = 0;
    for ($j = 0; $j < 6; $j++) {
      if (empty($row[2 + 2 * $j + 1]) || $row[2 + 2 * $j + 1] <= 0) {
        continue;
      }
      $penalty += $row[2 + 2 * $j + 1];
    }
    ...
    for ($j = 0; $j < 6; $j++) {
      ...
      if ($row[2 + 2 * $j + 1] != 0 && $row[2 + 2 * $j + 1] != -7759) {
        echo floor($row[2 + 2 * $j + 1] / 60);
      } else {
        echo '';
      }
      echo '</td>';
